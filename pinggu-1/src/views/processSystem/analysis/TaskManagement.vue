  ...
  <div class="subpage-container">
      ...
      <el-table :data="tableData" class="tech-table" style="width: 100%" v-loading="loading"
        ...
        <el-table-column label="操作" width="120" fixed="right">
          <template #default="{ row }">
            ...
            <el-button v-else link type="primary" @click="handleDetail(row)">详情</el-button> -->
    ...
    <el-dialog v-model="detailVisible" title="任务详情" width="700px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="评估工程">{{ detailData.projectName }}</el-descriptions-item>
        <el-descriptions-item label="关联表单ID">{{ detailData.formId }}</el-descriptions-item>
        <el-descriptions-item label="数据集ID">{{ detailData.datasetId }}</el-descriptions-item>
        <el-descriptions-item label="数据源类型">{{ detailData.sourceType }}</el-descriptions-item>
        <el-descriptions-item label="源格式">{{ detailData.sourceFormat }}</el-descriptions-item>
        <el-descriptions-item label="Cron表达式">{{ detailData.cronExpression || '-' }}</el-descriptions-item>
        <el-descriptions-item label="进度">{{ detailData.progressDone || 0 }} / {{ detailData.progressTotal || 0
        }}
          单位</el-descriptions-item>
        <el-descriptions-item label="状态">
          <el-tag size="small" :type="getStatusType(detailData.status)">{{ getStatusText(detailData.status)
          }}</el-tag>
...
import {
  getIntegrationTask, listIntegrationTasks,
...
const handleDetail = (row) => {
  getIntegrationTask(row.id).then(res => {
...
const handleTrigger = (row) => {
  ElMessageBox.confirm('确定要触发该任务执行吗？', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(() => {
    triggerIntegrationTask(row.id).then(() => {
      ElMessage.success('任务触发成功，正在执行中...')
      getTasks()
    })
  })
}

const handlePreprocess = (row) => {
  triggerIntegrationTask(row.id).then(() => {
    router.push('/process/process-sys/tasks?id=' + row.id)
    ElMessage.success('进入预处理页面')
  })
}
