<template>
    <div class="designer-container">
        <div class="designer-header">
            ...
            <div class="actions">
                <el-button-group>
                    <el-button icon="DocumentAdd" plain @click="saveDraft">保存模板</el-button>
                    <el-button icon="Check" type="primary" @click="handleGenerate">生成并发布</el-button>
                </el-button-group>
                <el-button icon="Close" circle style="margin-left: 12px;" @click="router.back()"></el-button>
            </div>
        </div>
        ...
        <div class="designer-body">
            ...
            <main class="designer-main">
                ...
                <div class="table-scroller">
                    <el-table :data="tableData" class="cyber-editable-table" border height="100%">
                        <el-table-column label="数据项名称" prop="name" min-width="150">
                            <template #default="{ row }">
                                <el-input v-model="row.name" placeholder="请输入数据项名称" />
                            </template>
                        </el-table-column>
                        <el-table-column label="数据源 ID" prop="dataSourceId" width="130">
                            <template #default="{ row }">
                                <el-input v-model="row.dataSourceId" placeholder="ID" />
                            </template>
                        </el-table-column>
                        <el-table-column label="关联表" prop="tableCode" width="150">
                            <template #default="{ row }">
                                <el-input v-model="row.tableCode" placeholder="表编码" />
                            </template>
                        </el-table-column>
                        <el-table-column label="关联字段" prop="fieldCode" width="150">
                            <template #default="{ row }">
                                <el-input v-model="row.fieldCode" placeholder="字段编码" />
                            </template>
                        </el-table-column>
                        <!-- <el-table-column label="时间范围" prop="timeRange" width="220">
                            <template #default="{ row }">
                                <el-date-picker v-model="row.timeRange" type="datetimerange" range-separator="至"
                                    start-placeholder="开始" end-placeholder="结束" size="small" />
                            </template>
                        </el-table-column> -->
                        <el-table-column label="关联指标" prop="indicator" min-width="180">
                            <template #default="{ row }">
                                <div class="indicator-bind-cell">
                                    <el-input v-model="row.indicator" readonly placeholder="点击绑定"
                                        @click="openIndicatorLib(row)" />
                        ...
                        <el-table-column label="数据约束 (任务阶段 | 时间范围)" min-width="380">
                            <template #default="{ row }">
                                <div class="constraint-edit-group">
                                    <el-select v-model="row.constraintTaskStage" placeholder="任务阶段" size="small"
                                        style="width: 140px">
                                        <el-option label="干扰执行阶段" value="干扰执行阶段" />
                                        <el-option label="情报收缴阶段" value="情报收缴阶段" />
                                        <el-option label="战前准备阶段" value="战前准备阶段" />
                                        <el-option label="效果评估阶段" value="效果评估阶段" />
                                    </el-select>
                                    <el-date-picker v-model="row.constraintTimeRange" type="datetimerange" size="small"
                        ...
                        <el-table-column label="操作" width="80" align="center" fixed="right">
                            <template #default="{ $index }">
                                <el-button icon="Delete" circle size="small" type="danger" link
        ...
        if (res.code === 200 || res.data) {
            ...
            if (data.items && Array.isArray(data.items)) {
                tableData.value = data.items.map(item => {
                    ...
                    let cTime = []
                    try {
                        if (item.dataConstraint) {
                            const config = JSON.parse(item.dataConstraint)
                            cStage = config.taskStage || ''
                            if (config.taskTimeStart && config.taskTimeEnd) {
                                cTime = [config.taskTimeStart, config.taskTimeEnd]
                            }
                        }
                    } catch (e) {
                        console.warn('Failed to parse dataConstraint:', e)
                    }
                    ...
                    return {
                        ...
                        timeRange: (item.timeRangeStart && item.timeRangeEnd) ? [item.timeRangeStart, item.timeRangeEnd] : [],
                        indicator: item.indicatorCode ? `${item.indicatorCode} - ${item.relatedIndicator || ''}` : '',
                        constraintTaskStage: cStage,
                        constraintTimeRange: cTime
                    }
                })
            }
...
:deep(.cyber-editable-table) {
    ...
    --el-table-header-bg-color: rgba(0, 242, 255, 0.1);
    ...
    .el-input__wrapper {
        background-color: rgba(6, 13, 23, 0.6) !important;
        box-shadow: 0 0 0 1px rgba(0, 242, 255, 0.1) inset !important;
        border: none;
        transition: all 0.3s;

        &:hover,
        &.is-focus {
            box-shadow: 0 0 0 1px #00f2ff inset, 0 0 10px rgba(0, 242, 255, 0.2) !important;
        }
    }
    ...
    // Date picker and select adjustments
    .el-range-input {
...
.table-scroller {
    ...
    .constraint-edit-group {
        ...
        :deep(.el-range-editor.el-input__inner) {
            background: rgba(6, 13, 23, 0.6) !important;
        }
    }
}
