...
function isDarkMode() {
  ...
  return !!document.querySelector('.dark-theme')
}
...
function mapDsToLocal(d) {
  const ft = Array.isArray(d.fields) ? d.fields.map(f => String(f || '').trim()).filter(Boolean).join(',') : ''
  return {
    name: d.name || '',
  ...
  graph.getNodes().forEach(node => {
    ...
    if (data.kind === 'data') {
      dataSources.push({
        ...
        fields: parseFields(data.fieldsText),
...
.flow-palette { border-right: 1px solid rgba(0, 242, 255, 0.14); }
.flow-inspector { border-left: 1px solid rgba(0, 242, 255, 0.14); }
.panel-title { display: flex; align-items: center; justify-content: space-between; gap: 8px; margin-bottom: 12px; font-size: 16px; font-weight: 700; color: #d8e7f5; }
.palette-card { display: flex; align-items: center; gap: 12px; padding: 12px; border: 1px solid rgba(0, 242, 255, 0.14); border-radius: 8px; background: rgba(10, 21, 37, 0.82); cursor: grab; user-select: none; transition: transform .18s ease, box-shadow .18s ease, border-color .18s ease; }
.palette-card + .palette-card { margin-top: 10px; }
.palette-card:hover { transform: translateY(-1px); box-shadow: 0 12px 28px rgba(0, 0, 0, .24); border-color: rgba(34, 211, 238, 0.46); }
...
.palette-name { font-weight: 700; color: #d8e7f5; }
.palette-desc, .palette-hint { margin-top: 3px; font-size: 12px; line-height: 1.45; color: #9fb6cb; }
.palette-hint { margin-top: 14px; padding: 10px; border-radius: 8px; background: rgba(0, 242, 255, 0.06); border: 1px solid rgba(0, 242, 255, 0.12); }
.palette-action { width: 100%; margin-top: 12px; }
.canvas-shell { display: flex; flex-direction: column; min-width: 0; }
.canvas-toolbar { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 14px; border-bottom: 1px solid rgba(0, 242, 255, 0.14); background: rgba(5, 13, 24, 0.96); font-weight: 700; color: #d8e7f5; }
.flow-canvas { flex: 1; min-height: 560px; }
.inspector-form :deep(.el-form-item) { margin-bottom: 14px; }
.inspector-form :deep(.el-form-item__label) { color: #9fb6cb; }
...
.param-drawer-meta { margin: 0 0 8px; color: #d8e7f5; font-weight: 600; }
.param-drawer-hint { margin: 0 0 12px; color: #9fb6cb; font-size: 13px; }
.param-desc { margin-left: 8px; font-size: 12px; color: #9fb6cb; }
