...
def calMain(datas, config):
    ...
    for idx, every_day_data in enumerate(all_days_datas):
        ...
        if len(todaytimes) == 0:
            rst.append("null")
            continue
        index_c = config["SAT_Charge"]
        index_b = config["SAT_Branch"]
        index_l = config["SAT_Loss"]
        index = config["SAT_Load"]
        if index_c == -1 and index_b == -1 and index_l == -1:
            y_SAT = abs(every_day_data[index])
            y_SA = list(np.zeros(len(todaytimes)))
            for i in range(len(every_day_data)):
                if i != index:
                    y_SA += abs(every_day_data[i])
            # 积分
            ...
            y_SAT_Charge = every_day_data[index_c]
            y_SAT_Load = every_day_data[index]
            y_SAT_Branch = every_day_data[index_b]
            y_SAT_Loss = every_day_data[index_b]
            y_SA = list(np.zeros(len(todaytimes)))
            y_SA_List = []
            for i in range(len(every_day_data)):
                if i != index and i != index_c and i != index_b and i != index_l:
                    y_SA += abs(every_day_data[i])
                    y_SA_List.append(y_SA)
            # 积分
            Q_SA = np.trapz(y_SA, np.array(todaytimes), axis=-1)
            if index != 1:
            ...
            if config["illumination_min"] != '' and config["illumination_max"] != '':
                ...
                y_i_idx = np.where(((illumination_min <= np.array(y_SA_List[0])) & (np.array(y_SA_List[0]) <= illumination_max)), 1, 0)
            elif config["illumination_min"] != '':
                illumination_min = eval(config["illumination_min"])
                y_i_idx = np.where(illumination_min <= np.array(y_SA_List[0]), 1, 0)
            elif config["illumination_max"] != '':
                illumination_max = eval(config["illumination_max"])
                y_i_idx = np.where(illumination_max >= np.array(y_SA_List[0]), 1, 0)
            else:
                y_i_idx = []
