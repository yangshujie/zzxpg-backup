 ...
 * 分页查询评估数据列表
 * @param {Object} query 查询参数
 * @param {number} [query.evaluationTaskId] 评估任务ID
 * @param {Array} [query.indicatorCodes] 评估指标标识列表
 * @param {string} [query.isAsc] 是否升序
 * @param {string} [query.orderBy] 排序字段
 * @param {string} [query.orderByColumn] 排序字段列名
 * @param {number} [query.pageNum] 页码
 * @param {number} [query.pageSize] 条数
 * @param {boolean} [query.reasonable] 是否合理性检查
 */
export function getPageEvaluationData(query) {
  return request({
    url: '/evaluationData/page',
    method: 'get',
    params: query
  })
}
 ...
 * @param {number|string} taskId
 */
export function getIntegrationTaskByRequirementCode(requirementCode) {
  return request({
    url: '/integration/task/by-requirement/' + requirementCode,
    method: 'get'
  })
}
 ...
 * 根据表和字段分页查询采集数据
 * @param {Object} query 
 * @param {Array<string>} query.fields 字段列表
 * @param {string} [query.isAsc] 
 * @param {string} [query.orderBy] 
 * @param {string} [query.orderByColumn] 
 * @param {number} [query.pageNum] 
 * @param {number} [query.pageSize] 
 * @param {boolean} [query.reasonable] 
 * @param {string} query.tableCode 表编码
 * @param {string} [query.taskTimeEnd] 任务结束时间，格式 yyyy-MM-dd HH:mm:ss 或 yyyy-MM-dd'T'HH:mm:ss
 * @param {string} [query.taskTimeStart] 任务开始时间，格式 yyyy-MM-dd HH:mm:ss 或 yyyy-MM-dd'T'HH:mm:ss
 */
export function queryTableDataPage(query) {
  return request({
    url: '/table-data/page',
    method: 'get',
    params: query
