  ...
  <div class="flow-runner-page">
    ...
    <div class="runner-header-v2">
      ...
      <div class="top-row">
        ...
        <div class="meta-info">
          ...
          <div class="meta-item">
            ...
            <span class="m-value">{{ indicatorSystemDetail?.systemName || indicatorSystemDetail?.indicatorSystemName || '—' }}</span>
          </div>
        </div>
      ...
      <div class="bottom-row">
        <el-steps :active="currentStepIdx" finish-status="success" align-center>
          <el-step v-for="(s, idx) in visibleSteps" :key="s.key" :status="stepStatus(s.key)">
            ...
            <template #title>
              <span class="step-label" :class="{ 'is-active': currentStep === s.key }" @click="canReviewStep(idx) && goToStep(idx)">
                {{ s.label }}
              </span>
            </template>
          </el-step>
      ...
      <div class="status-summary-bar">
        <div v-for="(s, idx) in visibleSteps" :key="'sum-' + s.key" 
        ...
        <div v-if="mode === 'execute'" class="calc-exec-panel">
          ...
          <div class="unified-exec-card-compact" :class="{ 'is-finished': calcFinished }">
            ...
            <div class="action-section">
              <el-button
                v-if="currentStep === 'comprehensiveCalc'"
                type="warning"
                size="small"
                icon="Refresh"
                :loading="apiLoading"
              ...
              <el-button type="primary" link icon="List" @click="calcLogDrawerVisible = true" class="log-btn">过程日志</el-button>
            </div>
          </div>
                  ...
                  <div class="trace-pane-content">
                    ...
                    <template v-if="selectedDetailNode.children?.length">
                      <div class="trace-section">
                        ...
                        <el-table :data="selectedDetailNode.children" size="small" border class="trace-table" show-summary :summary-method="getTraceTableSummary">
                          ...
                          <el-table-column prop="calculatedScore" label="评估分值" width="100">
                            <template #default="{ row }">
                              <span :class="getScoreClass(row.calculatedScore)" style="font-family: monospace; font-weight: bold;">
                          ...
                          <el-table-column label="贡献分值" width="100">
                            <template #default="{ row }">
                              <span style="color: var(--border-color-hover); font-weight: bold;">
                                {{ row.weight != null && row.calculatedScore != null ? (row.weight * row.calculatedScore).toFixed(2) : '—' }}
                              </span>
                            </template>
                          </el-table-column>
                        </el-table>
                        <div class="total-score-hint">
                          ...
                          <span class="value" :class="getScoreClass(selectedDetailNode.calculatedScore)">{{ selectedDetailNode.calculatedScore }}</span>
                        ...
                        <div class="method-card-v2">
                          <div class="m-header">
                            <div class="m-title-area">
                              <el-icon class="m-icon"><Setting /></el-icon>
                              <span class="m-name">{{ getAlgoInfo(selectedDetailNode.weightAssignAlgorithm).algorithmName }}</span>
                              <el-tag v-if="getAlgoInfo(selectedDetailNode.weightAssignAlgorithm).algorithmCode" size="small" effect="plain" class="m-code-tag">
                            ...
                            <div class="m-meta" v-if="getAlgoInfo(selectedDetailNode.weightAssignAlgorithm).author">
                              ...
                              <span class="value">{{ getAlgoInfo(selectedDetailNode.weightAssignAlgorithm).author }}</span>
                            </div>
                          </div>
                          ...
                          <div class="m-body">
                            ...
                            <p class="m-description">
                              {{ getAlgoInfo(selectedDetailNode.weightAssignAlgorithm).algorithmDescription || '通过指标间的相关性或专家赋权逻辑进行权重分配。' }}
                            </p>
                          </div>

                          <div class="m-footer" v-if="getAlgoInfo(selectedDetailNode.weightAssignAlgorithm).algorithmCodeUrl">
                            <div class="m-path">
                              <el-icon><Collection /></el-icon>
                              <span class="path-text">{{ getAlgoInfo(selectedDetailNode.weightAssignAlgorithm).algorithmCodeUrl }}</span>
                            </div>
                            <el-button v-if="getAlgoInfo(selectedDetailNode.weightAssignAlgorithm).id" 
                          ...
                          <div class="params-table-wrap" v-if="selectedDetailNode.weightAssignAlgorithmParams && Object.keys(selectedDetailNode.weightAssignAlgorithmParams).length">
                            <div class="sub-trace-title">执行参数配置</div>
                            <el-descriptions :column="2" border size="small">
                              <el-descriptions-item v-for="(val, key) in selectedDetailNode.weightAssignAlgorithmParams" :key="key" :label="key">
                      ...
                      <div class="trace-section">
                        ...
                        <div class="method-card-v2 aggregation">
                          <div class="m-header">
                            <div class="m-title-area">
                              <el-icon class="m-icon"><Refresh /></el-icon>
                              <span class="m-name">{{ getAlgoInfo(selectedDetailNode.conductionAlgorithm).algorithmName || '加权算术平均' }}</span>
                              <el-tag v-if="getAlgoInfo(selectedDetailNode.conductionAlgorithm).algorithmCode" size="small" effect="plain" class="m-code-tag">
                          ...
                          <div class="m-body">
                            ...
                            <p class="m-description">
                              {{ getAlgoInfo(selectedDetailNode.conductionAlgorithm).algorithmDescription || '基于下级指标的权重与分值乘积进行线性累加，计算公式：Score = Σ(ChildScore_i * Weight_i)' }}
                            </p>
                          </div>

                          <div class="m-footer" v-if="getAlgoInfo(selectedDetailNode.conductionAlgorithm).algorithmCodeUrl">
                            <div class="m-path">
                              <el-icon><Collection /></el-icon>
                              <span class="path-text">{{ getAlgoInfo(selectedDetailNode.conductionAlgorithm).algorithmCodeUrl }}</span>
                            </div>
                            <el-button v-if="getAlgoInfo(selectedDetailNode.conductionAlgorithm).id" 
                              type="primary" size="small" icon="View" 
                              class="m-preview-btn"
                    ...
                    <template v-else>
                      <div class="trace-section" v-if="selectedDetailNode.computeRule">
                        ...
                        <div
                          ...
                          @wheel.prevent="handleTraceFlowWheel"
                          @mousedown="startTraceFlowPan"
                          @dblclick="resetTraceFlowViewport"
                        >
                          <div
                            class="trace-flow-content"
                            :style="{
                          ...
                          >
                            <svg class="trace-flow-lines"
                                :width="selectedTraceFlowGraph.canvasWidth"
                              ...
                              <defs>
                                <marker id="trace-flow-arrow" markerWidth="12" markerHeight="12" refX="10" refY="6" orient="auto" markerUnits="strokeWidth">
                                  <path d="M0,0 L0,12 L10,6 z" />
                                </marker>
                              </defs>
                              <path
                                v-for="edge in selectedTraceFlowGraph.edges"
                                :key="edge.key"
                                class="trace-flow-edge"
                                :d="edge.path"
                              />
                            </svg>

                            <div
                              v-for="node in selectedTraceFlowGraph.nodes"
                              :key="node.id"
                            ...
                            >
                              <div class="step-box" :class="node.type">
                                <div class="step-icon">
                                  <el-icon v-if="node.type === 'start'"><Clock /></el-icon>
                                  <el-icon v-else-if="node.type === 'algo'"><Setting /></el-icon>
                                  <el-icon v-else-if="node.type === 'result'"><Medal /></el-icon>
                                </div>
                                <div class="step-main">
                                  <div class="step-head">
                                    <span class="step-type">{{ node.type === 'start' ? '原始数据源' : node.type === 'algo' ? '算法处理' : '计算结果' }}</span>
                                  </div>
                                  <div class="step-body">
                                    <template v-if="node.type === 'start'">
                                      ...
                                      <div v-if="formatDataSourceFields(node).length" class="field-chip-list">
                                        <el-tooltip
                                          v-for="field in formatDataSourceFields(node)"
                                          :key="field.key"
                                      ...
                                      <div v-if="formatTimeRange(node)" class="source-desc">时间：{{ formatTimeRange(node) }}</div>
                                    ...
                                    <template v-else-if="node.type === 'algo'">
                                      ...
                                      <div class="source-meta-grid">
                                        ...
                                        <span v-if="getAlgorithmFullInfo(node)?.algorithmCode">代码：<code>{{ getAlgorithmFullInfo(node).algorithmCode }}</code></span>
                                        <span v-if="getAlgorithmFullInfo(node)?.author">作者：{{ getAlgorithmFullInfo(node).author }}</span>
                                      </div>
                                      <div class="algo-url" v-if="node.url || getAlgorithmFullInfo(node)?.algorithmCodeUrl">路径：<code>{{ node.url || getAlgorithmFullInfo(node)?.algorithmCodeUrl }}</code></div>
                                      <div v-if="getAlgorithmFullInfo(node)?.algorithmDescription" class="source-desc">{{ getAlgorithmFullInfo(node).algorithmDescription }}</div>
                                      <div class="algo-params" v-if="getAlgorithmParams(node).length">
                                        <div class="param-tag-list">
                                          <el-tag v-for="p in getAlgorithmParams(node)" :key="p.field || p.name" size="small" effect="plain" type="info">
                                            {{ p.description || p.name || p.field }}: {{ p.defaultValue || '—' }}
                                          </el-tag>
                                        </div>
                                      </div>
                                      <el-button v-if="getAlgorithmPreviewId(node)" link type="primary" icon="View" size="small" class="inline-preview-btn" @click.stop="handlePreviewAlgo(getAlgorithmPreviewId(node))">算法预览</el-button>
                                    </template>
                                    <template v-else-if="node.type === 'result'">
                                      <div class="node-title">输出指标分值</div>
                                      <div class="result-label">当前得分：<strong>{{ selectedDetailNode.calculatedScore }}</strong></div>
                      ...
                      <div class="trace-section" v-if="selectedPreprocessPreview.length">
                        ...
                        <div class="preprocess-compare-panel">
                          ...
                          <div class="preprocess-card-grid">
                            <div v-for="field in selectedPreprocessPreview" :key="field.key" class="preprocess-field-card">
                              ...
                              <div class="curve-compare" v-loading="field.isFetching">
                                <template v-if="field.hasData">
                                  ...
                                  <template v-if="field.isNumeric">
                                    <div
                                      v-if="preprocessChartMode === 'overlay'"
                                      class="curve-chart"
                                      :class="{ 'is-panning': preprocessChartViewport.dragging }"
                                      :data-chart-height="300"
                                      :data-chart-top="field.chart.top"
                                      :data-chart-bottom="field.chart.bottom"
                                      @wheel.prevent="handlePreprocessChartWheel"
                                      @mousedown="startPreprocessChartPan"
                                      ...
                                      <svg viewBox="0 0 1200 300" class="axis-curve">
                                        <defs>
                                          <clipPath :id="`preprocess-chart-clip-${field.key}`">
                                            <rect :x="field.chart.left" :y="field.chart.top" :width="field.chart.right - field.chart.left" :height="field.chart.bottom - field.chart.top" />
                                          </clipPath>
                                          <linearGradient id="grad-before" x1="0%" y1="0%" x2="0%" y2="100%">
                                            <stop offset="0%" stop-color="var(--pre-chart-before)" stop-opacity="var(--pre-chart-before-area)" />
                                            <stop offset="100%" stop-color="var(--pre-chart-before)" stop-opacity="0" />
                                          </linearGradient>
                                          <linearGradient id="grad-after" x1="0%" y1="0%" x2="0%" y2="100%">
                                            <stop offset="0%" stop-color="var(--pre-chart-after)" stop-opacity="var(--pre-chart-after-area)" />
                                        ...
                                        <g class="chart-fixed-y">
                                          <g class="curve-grid y-grid">
                                            <line v-for="tick in getPreprocessViewportYTicks(field.chart)" :key="`y-${tick.y}`" :x1="field.chart.left" :x2="field.chart.right" :y1="tick.y" :y2="tick.y" />
                                          </g>
                                          <g class="curve-grid x-grid">
                                            <line v-for="tick in getPreprocessViewportXTicks(field.chart)" :key="`xg-${tick.index}`" :x1="tick.x" :x2="tick.x" :y1="field.chart.top" :y2="field.chart.bottom" />
                                          </g>
                                          <g class="curve-axis">
                                            <line :x1="field.chart.left" :x2="field.chart.left" :y1="field.chart.top" :y2="field.chart.bottom" />
                                            <line :x1="field.chart.left" :x2="field.chart.right" :y1="field.chart.bottom" :y2="field.chart.bottom" />
                                          </g>
                                          <g class="curve-labels y-labels">
                                            <text v-for="tick in getPreprocessViewportYTicks(field.chart)" :key="`yl-${tick.y}`" :x="field.chart.left - 9" :y="tick.y + 4">{{ tick.label }}</text>
                                          </g>
                                          <g class="curve-labels x-labels">
                                            <text v-for="tick in getPreprocessViewportXTicks(field.chart)" :key="`xl-${tick.index}`" :x="tick.x" :y="field.chart.bottom + 24">{{ tick.label }}</text>
                                          </g>
                                        </g>
                                        ...
                                        <g :clip-path="`url(#preprocess-chart-clip-${field.key})`">
                                          <g :transform="getPreprocessDataTransform(field.chart)">
                                            <polygon v-if="field.before.polyline" class="area-before" :points="`${field.before.polyline} ${field.chart.right},${field.chart.bottom} ${field.chart.left},${field.chart.bottom}`" fill="url(#grad-before)" />
                                            <polygon v-if="field.after.polyline" class="area-after" :points="`${field.after.polyline} ${field.chart.right},${field.chart.bottom} ${field.chart.left},${field.chart.bottom}`" fill="url(#grad-after)" />
                                            
                                            <polyline class="line-before" :points="field.before.polyline" />
                                            <polyline class="line-after" :points="field.after.polyline" />
                                            
                                            <g v-for="p in field.before.points" :key="`bp-${p.index}`" class="chart-node before">
                                              <ellipse :cx="p.x" :cy="p.y" :rx="getPreprocessPointRadius(3)" :ry="getPreprocessPointRadius(3)" />
                                              <title>{{ `预处理前\n值: ${p.value}\n时间: ${p.timestamp}` }}</title>
                                            </g>
                                            <g v-for="p in field.after.points" :key="`ap-${p.index}`" class="chart-node after">
                                              <ellipse :cx="p.x" :cy="p.y" :rx="getPreprocessPointRadius(3)" :ry="getPreprocessPointRadius(3)" />
                                              <title>{{ `预处理后\n值: ${p.value}\n时间: ${p.timestamp}` }}</title>
                                            </g>
                                          </g>
                                    ...
                                    <div v-else class="split-chart-grid">
                                      <div
                                        class="split-chart before"
                                        :class="{ 'is-panning': preprocessChartViewport.dragging }"
                                        :data-chart-height="160"
                                        :data-chart-top="field.beforeChart.top"
                                      ...
                                      >
                                        ...
                                        <svg viewBox="0 0 1200 160" preserveAspectRatio="none" class="axis-curve compact">
                                          <defs>
                                            <clipPath :id="`preprocess-before-clip-${field.key}`">
                                              <rect :x="field.beforeChart.left" :y="field.beforeChart.top" :width="field.beforeChart.right - field.beforeChart.left" :height="field.beforeChart.bottom - field.beforeChart.top" />
                                            </clipPath>
                                          </defs>
                                          <g class="curve-grid">
                                            <line v-for="tick in getPreprocessViewportYTicks(field.beforeChart)" :key="`by-${tick.y}`" :x1="field.beforeChart.left" :x2="field.beforeChart.right" :y1="tick.y" :y2="tick.y" />
                                            <line v-for="tick in getPreprocessViewportXTicks(field.beforeChart)" :key="`bxg-${tick.index}`" :x1="tick.x" :x2="tick.x" :y1="field.beforeChart.top" :y2="field.beforeChart.bottom" />
                                          </g>
                                          <g class="curve-axis">
                                            <line :x1="field.beforeChart.left" :x2="field.beforeChart.left" :y1="field.beforeChart.top" :y2="field.beforeChart.bottom" />
                                            <line :x1="field.beforeChart.left" :x2="field.beforeChart.right" :y1="field.beforeChart.bottom" :y2="field.beforeChart.bottom" />
                                          </g>
                                          <g class="curve-labels y-labels">
                                            <text v-for="tick in getPreprocessViewportYTicks(field.beforeChart)" :key="`byl-${tick.y}`" :x="field.beforeChart.left - 9" :y="tick.y + 4">{{ tick.label }}</text>
                                          </g>
                                          <g class="curve-labels x-labels compact-x-labels">
                                            <text v-for="tick in getPreprocessViewportXTicks(field.beforeChart, 4)" :key="`bxl-${tick.index}`" :x="tick.x" :y="field.beforeChart.bottom + 18">{{ tick.label }}</text>
                                          </g>
                                          <g :clip-path="`url(#preprocess-before-clip-${field.key})`">
                                            <g :transform="getPreprocessDataTransform(field.beforeChart)">
                                              ...
                                              <g v-for="p in field.beforeChart.points" :key="`bs-${p.index}`" class="chart-node before">
                                                <ellipse :cx="p.x" :cy="p.y" :rx="getPreprocessPointRadius(2.8)" :ry="getPreprocessPointRadius(2.8)" />
                                                <title>{{ `预处理前\n值: ${p.value}\n时间: ${p.timestamp}` }}</title>
                                              </g>
                                            </g>
                                          </g>
                                      ...
                                      <div
                                        class="split-chart after"
                                        :class="{ 'is-panning': preprocessChartViewport.dragging }"
                                        :data-chart-height="160"
                                        :data-chart-top="field.afterChart.top"
                                        ...
                                        <svg viewBox="0 0 1200 160" preserveAspectRatio="none" class="axis-curve compact">
                                          <defs>
                                            <clipPath :id="`preprocess-after-clip-${field.key}`">
                                              <rect :x="field.afterChart.left" :y="field.afterChart.top" :width="field.afterChart.right - field.afterChart.left" :height="field.afterChart.bottom - field.afterChart.top" />
                                            </clipPath>
                                          </defs>
                                          <g class="curve-grid">
                                            <line v-for="tick in getPreprocessViewportYTicks(field.afterChart)" :key="`ay-${tick.y}`" :x1="field.afterChart.left" :x2="field.afterChart.right" :y1="tick.y" :y2="tick.y" />
                                            <line v-for="tick in getPreprocessViewportXTicks(field.afterChart)" :key="`axg-${tick.index}`" :x1="tick.x" :x2="tick.x" :y1="field.afterChart.top" :y2="field.afterChart.bottom" />
                                          </g>
                                          <g class="curve-axis">
                                            <line :x1="field.afterChart.left" :x2="field.afterChart.left" :y1="field.afterChart.top" :y2="field.afterChart.bottom" />
                                            <line :x1="field.afterChart.left" :x2="field.afterChart.right" :y1="field.afterChart.bottom" :y2="field.afterChart.bottom" />
                                          </g>
                                          <g class="curve-labels y-labels">
                                            <text v-for="tick in getPreprocessViewportYTicks(field.afterChart)" :key="`ayl-${tick.y}`" :x="field.afterChart.left - 9" :y="tick.y + 4">{{ tick.label }}</text>
                                          </g>
                                          <g class="curve-labels x-labels compact-x-labels">
                                            <text v-for="tick in getPreprocessViewportXTicks(field.afterChart, 4)" :key="`axl-${tick.index}`" :x="tick.x" :y="field.afterChart.bottom + 18">{{ tick.label }}</text>
                                          </g>
                                          <g :clip-path="`url(#preprocess-after-clip-${field.key})`">
                                            <g :transform="getPreprocessDataTransform(field.afterChart)">
                                              ...
                                              <g v-for="p in field.afterChart.points" :key="`as-${p.index}`" class="chart-node after">
                                                <ellipse :cx="p.x" :cy="p.y" :rx="getPreprocessPointRadius(2.8)" :ry="getPreprocessPointRadius(2.8)" />
                                                <title>{{ `预处理后\n值: ${p.value}\n时间: ${p.timestamp}` }}</title>
                                              </g>
                                            </g>
                                  ...
                                  <div v-else class="non-numeric-compare">
                                    <div class="compare-table-grid">
                                      <div class="compare-col">
                                        <div class="col-title">预处理前 (预览 {{ field.beforeRecords.length }}/{{ field.beforeTotal }})</div>
                                      ...
                                      <div class="compare-col">
                                        <div class="col-title">预处理后 (预览 {{ field.afterRecords.length }}/{{ field.afterTotal }})</div>
              ...
              <el-table-column label="取值" min-width="260">
                <template #default="{ row }">
                  ...
                  <template v-else>
                    ...
                    <el-input v-else v-model="row.mappingValue" size="small" :placeholder="getStaticTextPlaceholder(row.key)"
                      @change="markReportConfigChanged" />
                  </template>
                </template>
              </el-table-column>
            </el-table>
    ...
    <el-dialog
      v-model="preprocessDataDetailVisible"
      :title="`预处理字段完整数据 - ${preprocessDataDetailField?.label || ''}`"
      fullscreen
      append-to-body
      destroy-on-close
      class="preprocess-data-dialog"
    >
      <div v-if="preprocessDataDetailField" class="preprocess-data-detail">
        <div class="detail-toolbar">
          ...
          <el-input
            v-model="preprocessDataDetailKeyword"
            clearable
            placeholder="搜索序号、值或时间"
            class="detail-search"
          />
        </div>
        ...
        <div class="detail-table-grid">
          <section class="detail-table-panel">
            ...
            <el-pagination
              v-model:current-page="preprocessDataDetailPage.before"
              v-model:page-size="preprocessDataDetailPage.pageSize"
              :page-sizes="preprocessDataDetailPageSizes"
              :total="preprocessDetailBeforeRows.length"
          ...
          <section class="detail-table-panel">
            ...
            <el-pagination
              v-model:current-page="preprocessDataDetailPage.after"
              v-model:page-size="preprocessDataDetailPage.pageSize"
              :page-sizes="preprocessDataDetailPageSizes"
              :total="preprocessDetailAfterRows.length"
              layout="total, sizes, prev, pager, next"
              background
              small
            />
    ...
    <!-- 源码预览对话框 -->
    <el-dialog v-model="algoPreviewVisible" :title="`算法源码预览 - ${currentPreviewAlgoName}`" width="70%" append-to-body destroy-on-close class="algo-preview-dialog">
      <div v-loading="algoPreviewLoading" class="algo-preview-container">
        <pre v-if="algoPreviewContent" class="algo-preview-pre"><code class="algo-preview-code" v-html="algoPreviewHighlightedHtml"></code></pre>
...
async function fetchPreprocessData(tableName, fieldName, indicatorCode) {
  ...
  try {
    const res = await axios.post(`${huageUrl}/listFieldMappingDataByTableFieldConditionsUsingPOST`, {
      fieldName,
      indicatorCode,
      ...
      .action-group {
        ...
        :deep(.el-button) {
          ...
          &.el-button--warning {
            ...
            &:hover { background: rgba(255, 170, 0, 0.25); }
          }
  ...
  .bottom-row {
    ...
    :deep(.el-steps) {
      ...
      .el-step__head {
        ...
        .el-step__line { 
          top: 16px; /* 32px icon / 2 = 16px center */
          background-color: var(--runner-table-border);
          height: 1px;
        }
      }
      ...
      .el-step.is-process {
        ...
        .el-step__title { color: var(--border-color-hover) !important; }
      }
      ...
      .el-step.is-success {
        ...
        .el-step__line-inner { border-color: var(--border-color-hover); }
      }
    }
...
/* 移除旧的单行工具栏样式 */
.runner-toolbar { display: none; }

/* 移除旧的区块样式 */
.runner-header, .runner-context, .runner-steps { display: none; }
...
.unified-exec-card-compact {
  ...
  .metrics-section {
    ...
    .el-divider { height: 30px; border-color: var(--runner-exec-divider); }
    ...
    .metric-item {
      ...
      .m-value {
        ...
        &.score { color: var(--runner-exec-accent); font-size: 16px; }
      }
      
      &.time {
        .m-value { font-size: 12px; font-weight: normal; color: var(--runner-exec-muted); }
      }
    }
  }
  ...
  .action-section {
    ...
    .log-btn {
      font-size: 13px;
      padding: 4px 8px;
      &:hover { background: var(--runner-exec-log-hover); }
    }
  }
}
...
.trace-pane-content {
  ...
  .total-score-hint {
    ...
    .value {
      ...
      font-weight: bold;
      font-family: 'Space Grotesk', monospace;
      &.is-excellent { color: #00f2ff; }
      &.is-good { color: #00d1ff; }
      &.is-pass { color: #ffd700; }
  ...
  .trace-flow-node {
    ...
    .step-box {
      ...
      &.start { border-left: 4px solid #10b981; }
      &.algo { border-left: 4px solid #00d1ff; }
      &.result { border-left: 4px solid var(--warning-color); }
    }
  ...
  .flow-chain {
    ...
    .step-box {
      ...
      &.start { border-left: 4px solid #10b981; }
      &.algo { border-left: 4px solid #00d1ff; }
      &.result { border-left: 4px solid var(--warning-color); background: rgba(var(--warning-color-rgb), 0.08); }
