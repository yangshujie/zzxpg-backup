<template>
  <div class="indicator-tree-workbench" :class="{ 'indicator-tree-workbench--fill': fillParentHeight }">
    ...
    <splitpanes class="workbench-split default-theme" :style="splitPaneStyle">
      <pane :size="variant === 'system' ? 22 : 32" :min-size="16">
      ...
      <pane :size="variant === 'system' ? 60 : 68" :min-size="28">
              ...
              <template v-if="variant === 'system' && !selectedNodeIsLeaf">
                <el-form-item label="赋权与传导">
                  <div class="node-algo-trigger-row">
                    <el-button type="primary" class="node-algo-btn" @click="openNodeLocalAlgoDialog">配置本节点算法</el-button>
                    <div class="node-algo-summary">
                      <el-tag type="info" effect="plain" class="node-algo-tag">权重：{{ workbenchNodeWeightSummary }}</el-tag>
                      <el-tag type="info" effect="plain" class="node-algo-tag">传导：{{ workbenchNodeConductionSummary }}</el-tag>
                    </div>
                  </div>
                </el-form-item>
              </template>
      ...
      <template v-if="selectedNode && !selectedNodeIsLeaf">
        <el-form label-width="120px" size="small">
          ...
          <template v-if="weightAssignParamsDef.length">
            ...
            >
              <el-input-number v-if="p.paramType === 'number'" v-model="localWeightAssignParams[p.paramField]" controls-position="right" style="width: 100%" />
              <el-input v-else v-model="localWeightAssignParams[p.paramField]" :placeholder="p.paramDesc" clearable />
            </el-form-item>
          </template>
          ...
          <template v-if="conductionParamsDef.length">
            <el-form-item
              v-for="p in conductionParamsDef"
              :key="p.paramField"
              :label="p.paramName || p.paramField"
              :required="p.requiredFlag === 1"
            >
              <el-input-number v-if="p.paramType === 'number'" v-model="localConductionParams[p.paramField]" controls-position="right" style="width: 100%" />
              <el-input v-else v-model="localConductionParams[p.paramField]" :placeholder="p.paramDesc" clearable />
            </el-form-item>
          </template>
        </el-form>
      </template>
...
class Graph extends G6Graph {
  ...
  setData(data, options = {}) {
    ...
    // 当需要保留位置时（如拖拽合并后），用现有节点的 x/y 覆盖布局算出来的新位置，
    // 这样画布不会因为布局重算而整体抖动，只有连线发生变化。
  ...
  render() {
    // G6 v4 的 super.render() 内部会调用 clear() -> initGroups() 重建根 group，
    // 这会把画布矩阵（缩放/平移）重置为单位矩阵，导致用户的视野丢失。
    // 这里在 super.render() 前后保存并恢复矩阵，保留用户当前的缩放与平移状态。
    const prevMatrix = this.get('group')?.getMatrix?.()
    const savedMatrix = prevMatrix ? [...prevMatrix] : null
    super.render()
    if (savedMatrix) {
      this.get('group')?.setMatrix?.(savedMatrix)
    }
...
/** 节点有了子节点后，清除仅叶节点可配的字段，并确保有非叶算法字段 */
function stripLeafOnlyFields(node) {
  ...
  if (props.variant === 'system') {
    if (node.weightAssignAlgorithm == null) {
      node.weightAssignAlgorithm = undefined
      node.weightAssignAlgorithmParams = {}
    }
    if (node.conductionAlgorithm == null) {
      const c = coerceConductionMethod(props.conductionAlgorithm)
      node.conductionAlgorithm = /^\d+$/.test(c) ? Number(c) : c
      node.conductionAlgorithmParams = {}
    }
  }
}

/** 节点失去所有子节点后，清除算法字段 */
function stripNonLeafAlgoFields(node) {
...
const localWeightAssignParams = ref({})
const localConduction = ref('')
const localConductionParams = ref({})

// 算法参数定义
const weightAssignParamsDef = ref([])
const conductionParamsDef = ref([])
const algoLoading = ref(false)

async function fetchAlgoParamsDef(id, category) {
...
watch(localWeightAssign, async (id) => {
  weightAssignParamsDef.value = await fetchAlgoParamsDef(id)
})

watch(localConduction, async (id) => {
  conductionParamsDef.value = await fetchAlgoParamsDef(id)
})
...
function openNodeLocalAlgoDialog() {
  ...
  localWeightAssignParams.value = { ...state.weightAssignAlgorithmParams }
  localConduction.value = state.conductionAlgorithm
  localConductionParams.value = { ...state.conductionAlgorithmParams }
...
function closeNodeLocalAlgoDialog() {
  applyNodeAlgoDialogState(selectedNode.value, {
    ...
    weightAssignAlgorithmParams: localWeightAssignParams.value,
    conductionAlgorithm: localConduction.value,
    conductionAlgorithmParams: localConductionParams.value
  })
...
const workbenchNodeWeightSummary = computed(() => {
  ...
  const algo = n.weightAssignAlgorithm
  if (algo) {
    return getZhpgWeightAssignLabel(algo, weightAssignOptions.value)
  }
  return '未配置'
})
...
const workbenchNodeConductionSummary = computed(() => {
  const n = selectedNode.value
  if (!n || selectedNodeIsLeaf.value) return '—'
  const localC = coerceConductionMethod(n.conductionAlgorithm)
  const globalC = coerceConductionMethod(props.conductionAlgorithm)
  const c = localC || globalC
  if (c) {
...
function clearWorkbenchNodeAlgoOverride() {
  ...
  localWeightAssignParams.value = {}
  localConduction.value = ''
  localConductionParams.value = {}
...
function findParentNodeByUid(nodes, uid) {
  for (const n of nodes || []) {
    if (n.children?.some(c => c.uid === uid)) return n
    const nested = findParentNodeByUid(n.children, uid)
...
function deleteNodePromoteChildren(uid) {
  ...
  // 如果父节点列表变为空（虽然 promote 通常意味着有子节点，但以防万一），处理父节点状态
  // 注意：pos.list 通常是某个父节点的 children 数组
  // 我们需要找到持有该 list 的 parent 对象
  const parentNode = findParentNodeByUid(treeData.value, uid)
  if (parentNode && (!parentNode.children || parentNode.children.length === 0)) {
    stripNonLeafAlgoFields(parentNode)
  }
...
function removeTreeNode(node, data) {
  ...
  if (index > -1) {
    ...
    // 如果父节点变为空（叶子），则清理掉算法字段
    if (parent.data && (!children || children.length === 0)) {
      stripNonLeafAlgoFields(parent.data)
    }
  }
}
