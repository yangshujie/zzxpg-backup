<template>
    <div class="subpage-container">
        <div class="page-header">
            ...
            <div class="right">
                <el-button type="success" icon="Back" @click="handleBack" style="margin-right: 12px;">返回</el-button>
                <el-button type="success" icon="Position" @click="handleBatchIssue"
        ...
        <div class="filter-bar tech-card">
            ...
            <el-radio-group v-model="filterStatus" class="tech-radio" style="margin-right: 20px;" @change="fetchList">
                ...
                <el-table-column label="下发单位" min-width="150">
                    <template #default="{ row }">
                        <div v-if="row.issuingUnit" class="unit-tags" @click="openSourceSelector(row)">
                            <el-tag size="small">{{ issuingUnitLabel(row.issuingUnit)
                            }}</el-tag>
...
const handleBack = () => {
    router.push('/process/analysis-sys/tasks')
}
    ...
    ElMessageBox.confirm(`确定要下发表单「${row.formName || row.name}」吗？`, '提示', {
        ...
        if (row.status !== 'COMPLETED') {
            issueForm(row.id).then(() => {
                ...
                router.push('/process/analysis-sys/task-detail?id=' + row.requireCode || 2042492758045360000)
            })
        } else {
            router.push('/process/analysis-sys/task-detail?id=' + row.requireCode || 2042492758045360000)
        }
...
const handleBatchIssue = () => {
    ...
    ElMessageBox.confirm(`确定要下发选中的 ${selectedIds.value.length} 个表单吗？`, '提示', {
        ...
        batchIssueForms({ ids: selectedIds.value }).then(() => {
            ...
            // fetchList()
            router.push('/process/analysis-sys/task-detail?id=' + searchRequireCode.value || 2042492758045360000)
        })
    })
}
...
:deep(.tech-table) {
    ...
    .el-table__row:hover {
        background: rgba(var(--border-color-hover-rgb), 0.02) !important;
    }
}
...
.unit-tags {
    ...
    .el-tag {
        ...
        border: 1px solid rgba(var(--border-color-hover-rgb), 0.3);
...
:deep(.tech-radio) {
    .el-radio-button__inner {
        background: rgba(var(--sider-bg-color-rgb), 0.8) !important;
        border: 1px solid rgba(var(--border-color-hover-rgb), 0.15) !important;
        color: var(--text-color-secondary);
    }

    .el-radio-button__original-radio:checked+.el-radio-button__inner {
        background: var(--border-color-hover) !important;
        color: #000;
        box-shadow: 0 0 10px rgba(var(--border-color-hover-rgb), 0.5);
    }
}

.expand-content {
    padding: 15px 50px;
    background: rgba(var(--sider-bg-color-rgb), 0.5);
    border-radius: 4px;
    color: var(--text-color-secondary);
    font-size: 14px;
...
:deep(.fill-dialog) {
    ...
    .source-list {
        ...
        overflow-y: auto;
        padding: 10px;
        background: rgba(var(--sider-bg-color-rgb), 0.4);
        border-radius: 4px;
    }
...
:deep(.tech-checkbox-group) {
    .el-checkbox {
        ...
        &:hover {
            background: rgba(var(--border-color-hover-rgb), 0.05);
        }
    ...
    .el-checkbox__inner {
        ...
        border-color: rgba(var(--border-color-hover-rgb), 0.5);
    }
...
.tech-select {
    :deep(.el-input__wrapper) {
        background: rgba(var(--sider-bg-color-rgb), 0.4) !important;
