<template>
  <div class="app-container">
                ...
                >
                  ...
                  <template v-if="form.workMode === ZHPG_WORK_MODE.MAIN_BRANCH_COLLAB">
                    <el-form-item label="构建阶段" class="zhpg-sidebar-form-item">
                      <div class="zhpg-build-phase-row">
                        <el-tooltip
                          v-if="isCurrentInitialDraft"
                          content="仅需构建上层结构，底层节点可不配算法。分系统通过接口拉取当前指标树；回传接口写入后将进入「已回传细化」"
                          placement="top"
                          :show-after="200"
                          effect="light"
                        >
                          <el-tag :type="getZhpgBuildPhaseTagType(form.buildPhase || 'INITIAL_DRAFT')" class="zhpg-phase-tag">
                            {{ getZhpgBuildPhaseLabel(form.buildPhase || 'INITIAL_DRAFT') }}
                          </el-tag>
                        </el-tooltip>
                        <el-tag v-else :type="getZhpgBuildPhaseTagType(form.buildPhase || 'INITIAL_DRAFT')" class="zhpg-phase-tag">
                          {{ getZhpgBuildPhaseLabel(form.buildPhase || 'INITIAL_DRAFT') }}
                        </el-tag>
                        <span v-if="isMainBranchRefined(form.workMode, form.buildPhase) && form.sourceSubsystem" class="zhpg-phase-source">
                          来源：{{ form.sourceSubsystem }}
                        </span>
                      </div>
                      <div v-if="isMainBranchRefined(form.workMode, form.buildPhase)" class="zhpg-phase-hint zhpg-phase-hint--success">
                        可直接编辑、计算权重与启用管理。
                      </div>
                    </el-form-item>
                  </template>
    ...
    <!-- 从模板创建对话框 -->
    <el-dialog title="从模板创建指标体系" v-model="templateDialogVisible" width="600px" append-to-body>
    ...
    <!-- 从指标库导入对话框 -->
    <el-dialog title="从指标库导入" v-model="importIndicatorVisible" width="600px" append-to-body>
    ...
    <!-- 从体系模板导入对话框 -->
    <el-dialog title="从体系模板导入指标树" v-model="importTemplateVisible" width="600px" append-to-body>
      <el-form label-width="100px">
    ...
    <el-dialog title="体系全局 · 赋权与传导" v-model="globalAlgoDialogVisible" width="520px" append-to-body>
