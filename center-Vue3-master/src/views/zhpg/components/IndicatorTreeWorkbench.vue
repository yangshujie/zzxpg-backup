...
/** 普通指标节点固定占位：长名称只影响省略显示，不改变布局宽度（避免挤压、抖动） */
const GRAPH_NODE_FIXED_W = 200
const GRAPH_NODE_FIXED_H = 72
/** 图形区标题最大字符（约两行内），超出省略号 */
const GRAPH_TITLE_MAX_CHARS = 22
...
function truncateGraphTitle(title) {
  ...
  if (t.length <= GRAPH_TITLE_MAX_CHARS) return t
  return `${t.slice(0, GRAPH_TITLE_MAX_CHARS - 1)}…`
}
...
function toGraphData(roots) {
  ...
  return treeToGraphData(td, {
    getNodeData: (datum, depth) => {
      ...
      if (dark) {
        /* 暗色：半透明深底 + 霓虹描边 + 发光 */
        labelColor = isEmpty ? '#6e85a5' : isVroot ? '#e2e8f0' : '#f1f5f9'
        nodeFill = isEmpty
          ? 'rgba(15,23,42,0.6)'
          : isVroot ? 'rgba(15,30,55,0.95)'
          : hexToRgba(accent, depth === 0 ? 0.18 : isLeaf ? 0.10 : 0.14)
        ...
        /* 亮色：渐变填充卡片 + 彩色粗描边 + 彩色投影 */
        labelColor = isEmpty ? '#94a3b8' : isVroot ? '#ffffff' : '#1e293b'
        nodeFill = isEmpty
          ? '#f1f5f9'
          : isVroot ? accent
          : hexToRgba(accent, 0.06)    /* 浅色彩色底，不是纯白 */
...
function initGraph() {
  ...
  graph.on(NodeEvent.DRAG, e => {
    const x = e.canvas?.x ?? e.viewport?.x ?? 0
    const y = e.canvas?.y ?? e.viewport?.y ?? 0
  ...
  graph.render().then(async () => {
    await graph?.fitView({}, false).catch(() => {})
...
.indicator-tree-workbench {
  --itw-bg: var(--mc-panel-bg-soft, linear-gradient(180deg, #f8fbff, #f0f6ff));
  --itw-pane-bg: var(--mc-surface, #ffffff);
  --itw-graph-bg: var(--mc-bg-elevated, #f8fafc);
  --itw-border: var(--el-border-color-light, #dbe4f0);
  --itw-border-accent: var(--mc-border, rgba(116, 149, 192, 0.22));
  --itw-accent: var(--mc-primary-strong, #0ea5e9);
  --itw-text: var(--mc-text-primary, #1e293b);
  --itw-text-secondary: var(--mc-text-secondary, #64748b);
  --itw-text-muted: var(--mc-text-tertiary, #94a3b8);
  --itw-node-label: var(--mc-text-primary, #1e293b);
  --itw-shadow: var(--mc-shadow, 0 4px 24px rgba(0, 0, 0, 0.06));
  --itw-grid-line: var(--mc-grid-line, rgba(99, 102, 241, 0.06));
