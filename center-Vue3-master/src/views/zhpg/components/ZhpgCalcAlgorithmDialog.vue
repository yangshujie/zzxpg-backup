              ...
              <el-form-item label="字段">
                <div class="fields-tag-container is-readonly">
                  <el-tooltip
                    ...
                    :key="idx"
                    :content="findTechnicalNameByLabel(selectedNodeData, f)"
                    placement="top"
                  >
                    <el-tag
                      size="small"
...
function nodeStyle(kind) {
  if (kind === 'data') return { fill: '#f0f9eb', stroke: '#67c23a' }
  if (kind === 'algorithm') return { fill: '#ecf5ff', stroke: '#409eff' }
  return { fill: '#fdf6ec', stroke: '#e6a23c' }
}
...
function addFlowNode(kind, x, y, data = {}, id = undefined) {
  ...
  return graphRef.value.addNode({
    ...
    x,
    y,
      width: kind === 'result' ? 150 : 240,
      height: kind === 'result' ? 64 : 88,
    attrs: {
...
function initGraph() {
  ...
  graphRef.value = new Graph({
    ...
    grid: { size: 12, visible: true, type: 'mesh', args: { color: '#e8edf5', thickness: 1 } },
    panning: true,
...
function applyDistributedEdgePorts() {
  ...
  distributed.forEach(item => {
    item.edge.setData({ ...(item.edge.getData() || {}), targetPortId: item.targetPortId }, { deep: false })
  })
}
...
function tidyCanvasEdges() {
  ...
  readGraphEdges().forEach((item, index) => {
    ...
    item.edge.setData({ ...(item.edge.getData() || {}), ...sides, targetPortId }, { deep: false })
    item.edge.setVertices(buildOrthogonalEdgeVertices(
      sourceBox,
      { ...targetBox, portId: targetPortId, sides },
      index
    ))
  })
}
...
function mapDsToLocal(d) {
  // 提取展示文本：优先取对象中的值（注释），否则取本身
  const ft = Array.isArray(d.fields) 
    ? d.fields.map(f => {
        if (f && typeof f === 'object') return Object.values(f)[0]
  ...
  return {
    ...
    fieldsText: ft,
    // 暂存原始字段对象数组，以便序列化时还原
    _rawFields: Array.isArray(d.fields) ? d.fields : [],
...
/** 通过显示的 Label 反查技术字段名（用于 Tooltip） */
function findTechnicalNameByLabel(nodeData, label) {
  if (!nodeData || !nodeData._rawFields) return '未知'
  const match = nodeData._rawFields.find(f => Object.values(f)[0] === label)
  return match ? Object.keys(match)[0] : label
}
...
function collectWorkspaceFromGraph() {
  ...
  graph.getNodes().forEach(node => {
    ...
    const pos = node.position()
    if (data.kind === 'data') {
      // 优先从 _rawFields 还原，如果手动修改了 fieldsText 则兜底回字符串数组
      let fields = data._rawFields || []
      const currentText = (data.fieldsText || '').trim()
      if (currentText) {
        const labels = currentText.split(/[,，]/).map(s => s.trim()).filter(Boolean)
        // 如果展示文本的数量与原始对象不一致，说明可能被手动删减/修改了，这里简单处理：保留匹配的对象
        if (labels.length !== fields.length) {
          fields = labels.map(label => {
            const found = fields.find(f => Object.values(f)[0] === label)
            return found || label
          })
        }
      } else {
...
.topbar-label { font-size: 15px; font-weight: 600; color: #606266; }
.topbar-tip { font-size: 13px; color: #909399; }
.algorithm-config { display: grid; grid-template-columns: 220px minmax(520px, 1fr) 320px; height: calc(100vh - 240px); min-height: 520px; border: 1px solid var(--el-border-color-lighter); border-radius: 10px; overflow: hidden; background: #f8fbff; }
.flow-palette, .flow-inspector { display: flex; flex-direction: column; padding: 14px 16px 30px; background: #fff; overflow-y: auto; overflow-x: hidden; height: 100%; box-sizing: border-box; }
...
.panel-title { display: flex; align-items: center; justify-content: space-between; gap: 8px; margin-bottom: 12px; font-size: 16px; font-weight: 700; color: #303133; }
.palette-card { display: flex; align-items: center; gap: 12px; padding: 12px; border: 1px solid var(--el-border-color-lighter); border-radius: 12px; background: #fff; cursor: grab; user-select: none; transition: transform .18s ease, box-shadow .18s ease, border-color .18s ease; }
.palette-card + .palette-card { margin-top: 10px; }
.palette-card:hover { transform: translateY(-1px); box-shadow: 0 10px 24px rgba(31, 45, 61, .08); border-color: #9fc8ff; }
...
.palette-name { font-weight: 700; color: #303133; }
.palette-desc, .palette-hint { margin-top: 3px; font-size: 12px; line-height: 1.45; color: #909399; }
.palette-hint { margin-top: 14px; padding: 10px; border-radius: 10px; background: #f5f7fa; }
.palette-action { width: 100%; margin-top: 12px; }
.canvas-shell { display: flex; flex-direction: column; min-width: 0; }
.canvas-toolbar { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 14px; border-bottom: 1px solid var(--el-border-color-lighter); background: rgba(255,255,255,.86); font-weight: 700; color: #303133; }
.flow-canvas { flex: 1; min-height: 560px; }
.inspector-form { padding-bottom: 30px; }
.inspector-form :deep(.el-form-item) { margin-bottom: 16px; }
...
.param-drawer-meta { margin: 0 0 8px; color: #303133; font-weight: 600; }
.param-drawer-hint { margin: 0 0 12px; color: #909399; font-size: 13px; }
.param-desc { margin-left: 8px; font-size: 12px; color: #909399; }
...
<style>
.zhpg-calc-algorithm-dialog.el-dialog { width: min(96vw, 1480px) !important; max-width: 1480px; margin-top: 4vh !important; }
.zhpg-calc-algorithm-dialog .el-dialog__body { padding: 12px 16px 8px; max-height: calc(100vh - 140px); overflow: hidden; }
</style>
