...
public class PlaceholderService {
    ...
    public void validateTemplateTokens(String htmlContent) {
        ...
        if (openCount != closeCount) {
            throw new ServiceException("占位符括号未正确闭合");
        }
        ...
        while (tokenMatcher.find()) {
            ...
            if ("/each".equals(token)) {
                loopDepth--;
                if (loopDepth < 0) throw new ServiceException("存在非法 /each 标签");
                continue;
            }
            ...
            if ("else".equals(token)) {
                if (ifDepth <= 0) throw new ServiceException("存在非法 else 标签");
                continue;
            }
            if ("/if".equals(token)) {
                ifDepth--;
                if (ifDepth < 0) throw new ServiceException("存在非法 /if 标签");
                continue;
            }
            if (token.matches("=.+")) continue;
            if (!token.matches("[a-zA-Z0-9_\\.]+")) {
                throw new ServiceException("存在不支持的占位符: {{" + token + "}}");
            }
        }

        if (loopDepth != 0) throw new ServiceException("#each 与 /each 数量不匹配");
        if (ifDepth != 0) throw new ServiceException("#if 与 /if 数量不匹配");
    }
